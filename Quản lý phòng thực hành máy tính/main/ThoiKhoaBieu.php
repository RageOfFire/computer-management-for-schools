...
<body>
    <main>
      ...
      <table class="table table-hover" style="height: 100%;">
        ...
        <tbody>
          <?php
          $xmlname = 'ThoiKhoaBieu';
          $xml = simplexml_load_file ('../XML/'.$xmlname.'.xml') or die ('Error');
          foreach ($xml->children() as $xmllist) {
            echo '<tr style="height: 100px;">';
            echo '<td>'.$xmllist -> MaTKB.'</td>';
            echo '<td>'.$xmllist -> PhongMay.'</td>';
            echo '<td>'.$xmllist -> GiangVien.'</td>';
            echo '<td>'.$xmllist -> MonHoc.'</td>';
            echo '<td>'.$xmllist -> ThoiGianHoc.'</td>';
            echo '<td>'.$xmllist -> Ngay.'</td>';
            echo '<td>'.$xmllist -> TongSoTiet.'</td>';
            echo '<td><a href="../php/deleteThoiKhoaBieu.php?s='.$xmllist -> PhongMay.'"><button class="btn btn-danger"><i class="fas fa-trash"></i></button></a></td>';
            echo '<td><a href="../main/editThoiKhoaBieusite.php?s='.$xmllist -> MaTKB.'"><button class="btn btn-success"><i class="far fa-edit"></i></button></a></td>';
            echo '</tr>';
          }
          ?>
        </tbody>
  ...
  <div class="container-fluid">
      <form class="d-flex" action="./searchThoiKhoaBieu.php" method="post">
